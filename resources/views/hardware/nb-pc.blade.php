<x-app-layout>
    ...
    <div class="py-12" x-data="{ showForm: false }"> <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div x-show="showForm" 
                 ...
                 style="display: none;"> <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-bold dark:text-white uppercase tracking-wider text-blue-600">Add New Asset</h3>
                    <button @click="showForm = false" class="text-gray-400 hover:text-red-500 text-2xl">&times;</button>
                </div>
                ...
                <form action="{{ route('hardware.nb-pc.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    @csrf
                    <input type="text" name="item_name" placeholder="Item Name / Class" class="rounded-lg border-gray-300 dark:bg-gray-700 dark:text-white shadow-sm" required>
...
<div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 border border-gray-200 dark:border-gray-700">
    <div class="flex items-center justify-between mb-6">
        ...
        <h3 class="text-lg font-bold dark:text-white uppercase tracking-wider">
            Notebook & PC List
        </h3>
        ...
        <div class="flex items-center gap-3">
            <button @click="showForm = !showForm" class="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg text-sm shadow-md flex items-center gap-2 transition font-semibold">
                ...
                <svg x-show="showForm" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" style="display: none;" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12" />
                </svg>
                <span x-text="showForm ? 'Close Form' : 'Add Asset'"></span>
            </button>

            <button onclick="exportTableToExcel('asset-table', 'Inventory-Notebook-PC')" class="bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded-lg text-sm shadow-md flex items-center gap-2 transition font-semibold">
                ...
                <div class="overflow-x-auto">
                    <table id="asset-table" class="w-full text-sm text-left border-collapse border border-gray-200 dark:border-gray-700">
                        ...
                        <tbody class="dark:text-gray-300">
                            ...
                            <tr class="{{ $key % 2 == 0 ? 'bg-gray-50 dark:bg-gray-700/50' : 'bg-white dark:bg-gray-800' }} hover:bg-blue-50 dark:hover:bg-gray-600 transition">
                                ...
                                <td class="p-3 border border-gray-200 dark:border-gray-700">{{ $asset->item_name }}</td>
                                ...
                                <td class="p-3 border border-gray-200 dark:border-gray-700 text-center">
                                    <div class="flex justify-center items-center gap-3">
                                        <a href="{{ route('hardware.nb-pc.edit', $asset->id) }}" class="text-blue-600 hover:text-blue-800 font-bold transition">EDIT</a>
                                        <form action="{{ route('hardware.nb-pc.destroy', $asset->id) }}" method="POST" onsubmit="return confirm('Hapus asset?')">
                                            @csrf @method('DELETE')
                            ...
                            @empty
                            <tr><td colspan="11" class="p-6 text-center text-gray-500 italic">No assets found.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
