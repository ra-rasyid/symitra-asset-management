<x-app-layout>
    <x-slot name="header">
        <h2 class="font-medium text-lg text-slate-800 dark:text-white">
            {{ __('Home / Hardware Inventory / Other Devices') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div x-show="showForm" 
                ...
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-lg font-bold dark:text-white uppercase tracking-wider text-blue-600">Add New Other Device</h3>
                ...
                <form action="{{ route('hardware.others.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    @csrf
                    <input type="text" name="item_name" placeholder="Item Name (e.g. Router, Switch)" class="rounded-lg border-gray-300 dark:bg-gray-700 dark:text-white shadow-sm" required>
                ...
                <div class="flex items-center justify-between mb-6">
                    ...
                    <div class="flex items-center gap-3">
                        <button @click="showForm = !showForm" class="bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg text-sm shadow-md flex items-center gap-2 transition font-semibold">
                            ...
                            <span x-text="showForm ? 'Close Form' : 'Add Device'"></span>
                        </button>
                            ...
                            <tr class="{{ $key % 2 == 0 ? 'bg-gray-50 dark:bg-gray-700/50' : 'bg-white dark:bg-gray-800' }} hover:bg-blue-50 dark:hover:bg-gray-600 transition">
                                ...
                                <td class="p-3 border border-gray-200 dark:border-gray-700 text-center">
                                    <div class="flex justify-center items-center gap-3">
                                        <a href="{{ route('hardware.others.edit', $asset->id) }}" class="text-blue-600 hover:text-blue-800 font-bold transition">EDIT</a>
                                        <form action="{{ route('hardware.others.destroy', $asset->id) }}" method="POST" onsubmit="return confirm('Hapus device ini?')">
                                            @csrf
                                            @method('DELETE')
    ...
    function exportTableToExcel(tableID, filename = '') {
        ...
        var wb = XLSX.utils.table_to_book(cloneTable, { sheet: "SYMITRA Others" });
        var finalFileName = filename ? filename + '.xlsx' : 'Export_Other_Devices.xlsx';
