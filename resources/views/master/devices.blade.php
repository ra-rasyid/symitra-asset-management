<x-app-layout>
    <x-slot name="header">
        <h2 class="font-medium text-lg text-slate-800 dark:text-white uppercase tracking-tight">
            {{ __('Home / Master Data / Hardware & Other Devices') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div x-show="showForm" 
                ...
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-blue-600 font-black uppercase tracking-widest text-sm">Define New Device Type</h3>
                    <button @click="showForm = false" class="text-gray-400 hover:text-red-500 transition">&times;</button>
                </div>
                
                <form action="{{ route('master.devices.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @csrf
                    <input type="text" name="device_name" placeholder="Device Name (e.g. Router, Switch, Laptop)" class="rounded-lg border-gray-300 dark:bg-gray-700 dark:text-white shadow-sm focus:ring-blue-500" required>
            ...
            <div class="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="font-bold dark:text-white uppercase tracking-widest text-sm text-slate-500">Device Directory</h3>
                    <div class="flex gap-3">
                        <button @click="showForm = !showForm" class="bg-blue-600 text-white py-2 px-4 rounded-lg text-xs font-black transition flex items-center gap-2 shadow-md hover:bg-blue-700">
                            <span x-text="showForm ? '✖ CLOSE' : '➕ ADD DEVICE'"></span>
                        </button>
                        <button onclick="exportTableToExcel('device-table', 'Master-Devices')" class="bg-green-600 text-white py-2 px-4 rounded-lg text-xs font-black flex items-center gap-2 shadow-md hover:bg-green-700 transition">
                            📥 EXPORT EXCEL
                        </button>
                    </div>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table id="device-table" class="w-full text-sm text-left border-collapse border border-gray-200 dark:border-gray-700">
                        ...
                        <tbody class="dark:text-gray-200">
                            ...
                            <tr class="{{ $key % 2 == 0 ? 'bg-gray-50 dark:bg-gray-700/50' : 'bg-white dark:bg-gray-800' }} hover:bg-blue-50 dark:hover:bg-gray-600 transition">
                                ...
                                <td class="p-3 border border-gray-200 dark:border-gray-700 font-bold uppercase tracking-wide">{{ $dev->device_name }}</td>
                                <td class="p-3 border border-gray-200 dark:border-gray-700 text-center">
                                    <form action="{{ route('master.devices.destroy', $dev->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus tipe perangkat ini?')">
                                        @csrf 
                                        @method('DELETE')
    ...
    function exportTableToExcel(tableID, filename = '') {
        ...
        var wb = XLSX.utils.table_to_book(cloneTable, { sheet: "Device Types" });
        XLSX.writeFile(wb, (filename ? filename : 'Master-Devices') + '.xlsx');
    }
    </script>
</x-app-layout>